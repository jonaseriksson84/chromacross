...
function getUniqueLetters(word1: string, word2: string): string[] {
	return Array.from(new Set([...word1.split(""), ...word2.split("")])).sort();
}
...
function generatePuzzle(date: string): Puzzle {
	...
	for (let attempt = 0; attempt < maxAttempts; attempt++) {
		...
		if (validateUniqueness(intersection, WORD_LIST)) {
			console.log(
				`Found valid puzzle after ${attempt + 1} attempts: ${intersection.word1.toUpperCase()} / ${intersection.word2.toUpperCase()}`,
			);
	...
	console.warn(
		`Failed to generate puzzle after ${maxAttempts} attempts, using fallback`,
	);
	return {
		puzzleId: daysSinceEpoch,
		date,
...
export async function GET() {
	...
	console.log(
		"Generated puzzle:",
		puzzle.words.horizontal,
		"/",
		puzzle.words.vertical,
	);
